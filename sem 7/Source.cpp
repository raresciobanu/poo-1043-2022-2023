...
public:
	...
	Student(int idMatricol, GEN gen, int nrNote, double* note,
		const char* nume, int varsta, string telefon) :ID_MATRICOL(
			idMatricol > 0 ? idMatricol : throw new exception("idMatricol trebuie sa fie >0")
		) {
	...
	friend double medieNoteStudent(const Student& s);
	...
	void adaugaElementInVectorulDeNote(double nota) {
		double* paux = new double[nrNote + 1];
		...
		delete[] note; //evitare memory leaks

		note = paux;
	}

	void operator+=(double nota) {
		double* paux = new double[nrNote + 1];
	...
	void stergeUltimulElementDinVectorulDeNote() {
		double* paux = new double[nrNote - 1];
	...
	void stergeAparitiileUneiNoteDinVectorulDeNote(double nota) {
		int count = 0;
		for (int i = 0; i < nrNote; i++) {
			if (note[i] == nota) {
				count++;
		...
		if (count > 0) {
			double* paux = new double[nrNote - count];
	...
	void stergeNotaDeLaUnAnumitIndex(int index) {
		if (index >= 0 && index < nrNote) {
			double* paux = new double[nrNote - 1];
...
double medieNoteStudent(const Student& s) {
	double medie = 0.0;
	for (int i = 0; i < s.nrNote; i++) {
		medie += s.note[i];
	}
...
void main() {
	...
	Student s5(s4); //constr de copiere
	Student s6 = s4; //constr de copiere
	Student s7;
	s7 = s5; //op =
	...
	if (!s4) {
		...
		cout << "s4 nu are note in vectorul note\n\n";
	}
